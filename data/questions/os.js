Questions.register([
    ...
    },
    {
        ...
        "correctAnswer": 1,
        "explanation": {
            "solution": "Response Time (RT): Time from submission to FIRST CPU allocation (when process FIRST gets CPU, not completion). CRITICAL for interactive/time-sharing systems where users expect immediate feedback (typing, clicking). GATE formula: $RT = T_{first\_CPU} - T_{arrival}$. Contrast with TAT (arrival to completion). Example: Job arrives 0, first gets CPU at 8, completes at 20. RT=8, TAT=20. GATE comparison: (1) FCFS: Poor response for processes at end of queue, (2) SJF: Can have terrible response for long jobs (may wait forever), (3) Round Robin: BEST response time - all jobs get CPU within (n-1)*q, (4) SRTF: Good response for short jobs, poor for long. Interactive systems optimize response time over throughput.",
            "formula": "$RT = T_{first\_CPU} - T_{arr}$ (important for interactive)"
        }
    },
    {
        "id": "os2-sync-001",
